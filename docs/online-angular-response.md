...
The 16-to-64 rate disagreements at these four origins are 0.000157, 0.006269, 0.024847 and 0.019197 rad/s. Refinement leaves substantial forecast error; the refined trajectory is still model output, not plant truth. At row150 current-linear and delayed-linear pitch increments are +15.112 and +27.265rad/s, largely canceling the quadratic head. Signed trajectory accounting does not identify causal head error: deleting a head changes the integrated path. Local head derivatives freeze history, filters and hidden state; they do not establish recurrent stability.
