...
class FusedLinearizationSolver(RecoverySolver):
    """Share derivative rollout work while retaining baseline trial scoring."""

    def _linearization_rollout(self, model):
        alternate = FusedFeedbackSuffixPlan(
            **{field.name: getattr(model, field.name) for field in fields(model)}
        )
        # The callable takes each request's values explicitly, including its
        # frozen middle. It has no shared solver-kernel cache of its own.
        return alternate.rollout
...
def run(fixtures, records, output, *, scope="full"):
    ...
    solvers = [
        ...
        for cls in (
            RecoverySolver,
            FusedRecoverySolver if scope == "full" else FusedLinearizationSolver,
        )
    ]
    ...
    if scope == "full":
        ...
        assert solvers[0]._kernels is solvers[1]._kernels
    ...
    report = {
        ...
        "fusion_scope": scope,
        "strict_bitwise_required": scope == "linearization",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--scope", choices=("full", "linearization"), default="full")
    args = parser.parse_args()
    run(args.fixtures, args.records, args.output, scope=args.scope)
