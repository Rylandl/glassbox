...
uv run glassbox benchmark cascade-x8 artifacts/x8_cascade \
  --output artifacts/x8_cascade/cascade_report.json \
...
uv run glassbox benchmark cascade-x8 artifacts/x8_cascade --diagnose --split all \
...
baseline are exactly those of `glassbox evaluate --protocol x8`.
...
4. **The rotational dynamics disagree with the published derivatives, and it is not a data
   ...
   (`glassbox benchmark cascade-x8 --diagnose`, all 17 maneuvers, R² 0.79 roll and 0.84 pitch) with the
