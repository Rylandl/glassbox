...
The platform-neutral structured residual is trained at 0.1, 0.5, and 1.0 seconds with the shared long-rollout objective. The multirotor torque map is memoryless: control-generated torque follows the applied motor state with no lag of its own. One window budget, a window ceiling and an unrolled-transition ceiling per horizon, governs both what the fitter extracts and what one gradient step processes. This medium corpus is under it at every horizon, so it uses all 7,192 valid multi-horizon windows while larger or longer-horizon corpora remain bounded.
