...
def update_readout(
    ...
    diagonal = replay._curvature_diagonal(
...
class FastOnlineFit:
    ...
    def __init__(self, prefix):
        ...
        one_step = replay._windows(states, inputs, range(history, history + count), history, 1)
    ...
    def observe(self, index, command, next_observation):
        ...
        new = replay._windows(states, inputs, [model.history_steps], model.history_steps, self._horizon)
        recent = {
            key: np.concatenate((self._recent[key], new[key]))[-32:]
            for key in replay._FIELDS
        }
        data, weights = replay._full_cache(self._bootstrap, recent)
    ...
    def load(cls, path):
        try:
            ...
            for role in ("bootstrap", "recent"):
                setattr(obj, "_" + role, {
                    key: arrays[f"{role}_{key}"].copy() for key in replay._FIELDS
                })
            with jax.enable_x64(True):
    ...
    def _validate(self):
        ...
        if (
            ...
            or not np.array_equal(self._scale, replay._scale(self._bootstrap))
