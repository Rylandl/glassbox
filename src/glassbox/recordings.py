"""Uniformly sampled motion recordings, contiguous segments and window provenance."""
...
class SequenceSegment:
    """One contiguous recording: N+1 observations and N issued commands."""
    ...
    def __post_init__(self):
        ...
        if (
            ...
            or not self.recording_id.strip()
...
def segments_from_mask(recording_id, states, inputs, valid, *, dt_s):
    """Retain contiguous valid runs without padding or bridging missing observations."""
...
class SequenceWindows:
    """Extracted recording windows and their source row identities."""
    ...
    def __post_init__(self):
        ...
        validate_window_consistency(self.batch, keys, origins)
...
class SequenceCollection:
    ...
    def extract(self, keys, *, history_steps, horizon_steps):
        """Extract explicitly chosen windows, preserving recording boundaries and provenance."""
        keys = tuple(keys)
