"""Frozen public-baseline and incremental quadratic-mechanism comparisons."""
...
def reduce(summaries, protocol, *, rows):
    """Require broad gains against public and the predeclared mechanism repair.
    ...
    limits apply only to the public comparison; the bilinear comparison has its
    own three frozen limits, not a second application of those public limits.
    """
    ...
    bilinear = _comparison(rows, protocol, "bilinear")
    limits = protocol["decision"]["mechanism_comparison"]
    if limits["reference_arm"] != "bilinear":
        raise ValueError("frozen mechanism reference must be bilinear")
    targeted = [
        row
        for row in bilinear["physical_comparisons"]
    ...
    return {
        **public,
        ...
        "mechanism_comparison": {
            "reference_arm": "bilinear",
            "baseline_fields_mean": "bilinear comparator, not public baseline",
            **{key: bilinear[key] for key in descriptive},
