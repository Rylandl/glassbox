"""Frozen affine-centered quadratic initialization on identical excited recordings."""
...
def protocol():
    if digest(PROTOCOL) != PROTOCOL_SHA256:
        raise ValueError(
            "frozen excited affine-anchored quadratic experiment protocol changed"
        )
...
def _ablation_sources():
    paths = [
        ...
        for stem in (
            "affine_anchored_checkpoints",
            "affine_anchored_model",
            "affine_anchored_matching",
            "affine_anchored_experiment",
            "affine_anchored_decision",
        )
    ...
    if not all(path.is_file() for path in paths):
        raise ValueError(
            "affine-anchored quadratic experiment implementation or tests are missing"
        )
...
def _implementation():
    ...
    spec = importlib.util.spec_from_file_location(
        "glassbox.experimental._affine_anchored_interaction", inherited.__file__
    )
    ...
    def check(p):
        ...
        runtime["affine_anchored_implementation"] = _ablation_sources()
        runtime["checkpoint_source_sha256"] = _checkpoint_sources(
            p, runtime["affine_anchored_implementation"]
        )
        return runtime
...
def trusted_bilinear():
    """Verify the prior diagnostic evidence without using its test cohort for fitting."""
    p = protocol()
    declared = p["prior_bilinear_bundle"]
    path = Path(declared["path"])
    if digest(path / "run.json") != declared["manifest_sha256"]:
        raise ValueError("prior bilinear bundle differs from trusted anchor")
    manifest = verify_files(path, "run.json")
    if manifest["protocol_sha256"] != p["prior_bilinear_protocol"]["sha256"]:
        raise ValueError("prior bilinear protocol differs from trusted source")
    return path
...
def compare_candidate(model, simulator):
    from .affine_anchored_matching import check_matched_models
...
def fit_provenance(simulator, arm, output, runtime):
    ...
    result = {
        ...
        "diagnostic_bilinear_bundle_sha256": protocol()["prior_bilinear_bundle"][
            "manifest_sha256"
        ],
    }
    if arm != "baseline":
...
def _expected_recipe(arm):
    ...
    from .affine_anchored_model import RECIPE as anchored
    from .state_quadratic_model import RECIPE as quadratic

    return {"baseline": public, "candidate": anchored, "quadratic": quadratic}[arm]
...
def check_checkpoints(output, simulator, arm, *, replay=False):
    from .affine_anchored_checkpoints import check_checkpoint_links, replay_checkpoints
...
def fit_arm(simulator, arm, output):
    ...
    from glassbox.experimental.affine_anchored_model import (
        CandidateFitError as AnchoredFitError,
    )
    from glassbox.experimental.affine_anchored_model import fit_candidate
    ...
    from .affine_anchored_checkpoints import capture_fit, save_checkpoints
    ...
    try:
            ...
            if arm == "candidate":
                if not read_json(base / "baseline/outcome.json")["model_available"]:
                    raise PreparationUnavailable(
                        "current public-excited baseline unavailable; anchored quadratic optimizer not attempted"
                    )
        ...
        if not isinstance(
            error, (CandidateFitError, AnchoredFitError, ExcitationDataError)
...
def _check_fit_dependencies(outcomes):
    for arm in ("baseline", "quadratic"):
        if outcomes[arm]["status"] == "preparation_unavailable":
            raise ValueError("only candidate can have unavailable public preparation")
    baseline_available = outcomes["baseline"]["model_available"]
    candidate = outcomes["candidate"]
    unavailable = candidate["status"] == "preparation_unavailable"
    if unavailable == baseline_available or (
        not baseline_available and candidate["model_available"]
    ):
        raise ValueError("candidate attempt differs from current baseline availability")
...
def check_links(output, simulator, *, evaluation=False):
    ...
    check_import(simulator, output, runtime)
    base = Path(output) / simulator
    outcomes = {arm: read_json(base / arm / "outcome.json") for arm in ARMS}
    _check_fit_dependencies(outcomes)
    for arm, cls in _model_classes().items():
        directory = base / arm
        check_checkpoints(output, simulator, arm)
        if not outcomes[arm]["model_available"]:
            continue
    ...
    shared = shared_initialization(output, simulator)
    if evaluation:
        if (
            shared
            != read_json(Path(output) / "initialization-matching.json")[simulator]
        ):
            raise ValueError("shared affine initialization evidence differs")


def shared_initialization(output, simulator):
    from .affine_anchored_checkpoints import check_shared_initialization

    return check_shared_initialization(
        {arm: Path(output) / simulator / arm / "checkpoints" for arm in ARMS}
    )
...
def decision(output):
    from glassbox.experimental.affine_anchored_decision import reduce
...
def finalize(output):
    ...
    write_json(
        output / "initialization-matching.json",
        {sim: shared_initialization(output, sim) for sim in SIMULATORS},
    )
    ...
    sha = freeze_files(
        ...
        "run.json",
        {
            ...
            "scope": "Frozen matched affine-anchored-quadratic-v1 comparison",
            "runtime": check_sources(),
        },
    )
...
def replay(simulator, output, expected_sha256):
    ...
    result = {
        ...
        "shared_initialization": shared_initialization(output, simulator),
