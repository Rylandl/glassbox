"""Validate the unfitted published Cascade X8 against the NTNU flight campaign.

This is a fixed-wing corpus experiment, not a library component. It builds a
grid of published-parameter variants of the Cascade Skywalker X8, predicts the
untouched validation maneuvers with each, scores them under the X8 policy
against the same kinematic-persistence baseline the fitted models are scored
against, and regresses the one-step force and moment residuals on the flight
variables so a losing variant says which coefficient it is losing on. The plant
it drives lives in :mod:`glassbox.integrations.cascade`.
"""
