...
DESIGN = {
    ...
    "precision_rule": "After a finite feasible trial fails the unchanged Armijo test, stop backtracking if a checked current checkpoint exists, the successful bounded QP ray has no initial constraint violation, and its full linear predicted decrease is nonnegative but below one representable increment of the reported objective. Full steps still get evaluated and may be accepted. Separately stop if a candidate rounds exactly to the current evaluator input.",
