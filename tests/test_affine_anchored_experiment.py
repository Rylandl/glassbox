...
from glassbox.experimental import affine_anchored_experiment as experiment
...
def test_private_parent_binding_preserves_historical_module():
    ...
    assert experiment.protocol()["id"] == "affine-anchored-quadratic-v1"
...
def test_both_imports_checked_before_any_fit(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(experiment, "trusted_bilinear", lambda: tmp_path)
    calls = []
    monkeypatch.setattr(
        experiment,
        "check_import",
...
def test_candidate_matching_uses_trusted_same_data_reference(monkeypatch):
    from glassbox.experimental import affine_anchored_matching as matching
...
def test_links_keep_candidate_validation_when_fresh_quadratic_unavailable(
    ...
    monkeypatch.setattr(experiment, "trusted_bilinear", lambda: tmp_path)
    monkeypatch.setattr(experiment, "shared_initialization", lambda *a: {})
    ...
    for arm in experiment.ARMS:
        ...
        for filename, value in (
            (
                "outcome.json",
                {
                    "model_available": arm != "quadratic",
                    "status": "fit_failure" if arm == "quadratic" else "complete",
                },
            ),
            ("report.json", {}),
            ("contract.json", {}),
    ...
    experiment.write_json(tmp_path / "initialization-matching.json", {"cascade": {}})
    experiment.check_links(tmp_path, "cascade", evaluation=True)
    experiment.write_json(
        tmp_path / "initialization-matching.json", {"cascade": {"changed": True}}
    )
    with pytest.raises(ValueError, match="shared affine initialization"):
        experiment.check_links(tmp_path, "cascade", evaluation=True)
    experiment.write_json(
...
def fake_fit_environment(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        "data_ready",
        lambda output: {
            "affine_anchored_implementation": {"observer": "sha"},
...
def test_anchored_fit_consumes_current_baseline_once_without_preparation_refit(
    monkeypatch, tmp_path
):
    import glassbox
    ...
    fitted = []
    monkeypatch.setattr(
        anchored, "fit_candidate", lambda *a, **k: fitted.append((a, k)) or model
    )
    monkeypatch.setattr(
...
def test_unavailable_baseline_records_unattempted_anchored_preparation(
    monkeypatch, tmp_path
):
    from glassbox.experimental import affine_anchored_model as anchored
...
def test_only_expected_numeric_fit_errors_are_recorded(monkeypatch, tmp_path, arm):
    ...
    error = {
        "baseline": ValueError,
        "candidate": anchored.CandidateFitError,
    ...
    monkeypatch.setattr(anchored, "fit_candidate", fail)
...
def test_shared_initialization_checks_all_arm_directories(monkeypatch, tmp_path):
    from glassbox.experimental import affine_anchored_checkpoints as checkpoints
    ...
    monkeypatch.setattr(
        ...
        lambda paths: calls.append(paths) or {"shared": True},
    )
    assert experiment.shared_initialization(tmp_path, "cascade") == {"shared": True}
    assert calls == [
        {arm: tmp_path / "cascade" / arm / "checkpoints" for arm in experiment.ARMS}
    ]


def test_prior_diagnostic_bundle_cannot_be_locally_reanchored(monkeypatch, tmp_path):
    p = deepcopy(experiment.protocol())
    p["prior_bilinear_bundle"]["path"] = str(tmp_path)
    experiment.write_json(
        tmp_path / "run.json",
        {"files": {}, "protocol_sha256": p["prior_bilinear_protocol"]["sha256"]},
    )
    p["prior_bilinear_bundle"]["manifest_sha256"] = experiment.digest(
        tmp_path / "run.json"
    )
    monkeypatch.setattr(experiment, "protocol", lambda: p)
    assert experiment.trusted_bilinear() == tmp_path
    value = experiment.read_json(tmp_path / "run.json")
    value["protocol_sha256"] = "rewritten"
    experiment.write_json(tmp_path / "run.json", value)
    with pytest.raises(ValueError, match="trusted anchor"):
        experiment.trusted_bilinear()
    p["prior_bilinear_bundle"]["manifest_sha256"] = experiment.digest(
        tmp_path / "run.json"
    )
...
def test_finalization_binds_shared_initialization_before_decision(
    monkeypatch, tmp_path
):
    expected = {sim: {"simulator": sim} for sim in experiment.SIMULATORS}
    monkeypatch.setattr(
        experiment, "shared_initialization", lambda output, sim: expected[sim]
    )
    monkeypatch.setattr(experiment, "check_sources", lambda: {"pinned": True})

    def decision(output):
        assert experiment.read_json(output / "initialization-matching.json") == expected
        return {"qualified": False}

    monkeypatch.setattr(experiment, "decision", decision)
    digest = experiment.finalize(tmp_path)
    manifest = experiment.verify_files(tmp_path, "run.json")
    assert digest == experiment.digest(tmp_path / "run.json")
    assert "initialization-matching.json" in manifest["files"]
    with pytest.raises(FileExistsError):
        experiment.finalize(tmp_path)


@pytest.mark.parametrize(
    "baseline,candidate,quadratic,accepted",
    [
        ("complete", "complete", "complete", True),
        ("complete", "complete", "fit_failure", True),
        ("complete", "fit_failure", "complete", True),
        ("fit_failure", "preparation_unavailable", "complete", True),
        ("fit_failure", "complete", "complete", False),
        ("fit_failure", "fit_failure", "complete", False),
        ("complete", "preparation_unavailable", "complete", False),
        ("preparation_unavailable", "preparation_unavailable", "complete", False),
        ("complete", "complete", "preparation_unavailable", False),
    ],
)
def test_replay_requires_actual_current_baseline_preparation(
    baseline, candidate, quadratic, accepted
):
    outcomes = {
        arm: {"status": status, "model_available": status == "complete"}
        for arm, status in (
            ("baseline", baseline),
            ("candidate", candidate),
            ("quadratic", quadratic),
    ...
    if accepted:
        experiment._check_fit_dependencies(outcomes)
    else:
        with pytest.raises(ValueError, match=r"preparation|baseline availability"):
            experiment._check_fit_dependencies(outcomes)
