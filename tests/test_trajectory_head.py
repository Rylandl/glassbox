...
from screen_trajectory_head import TrajectoryReadout, correction, trajectory_residual
...
def test_trajectory_jacobian_and_dual(name, source, begin, first, dt, n_commands):
    ...
    with jax.enable_x64(True):
        ...
        truth = jnp.asarray(states[first])
        ...
        expected = -whitened.T @ np.linalg.solve(
            whitened @ whitened.T + np.eye(15), np.asarray(r)
        )
