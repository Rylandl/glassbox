...
from glassbox.experimental import command_excitation_experiment as experiment
...
def toy_evaluation(monkeypatch, tmp_path):
    ...
    for path in ("data", "baseline", "original", "candidate"):
...
def test_private_parent_binding_preserves_historical_module():
    ...
    assert experiment.protocol()["id"] == "independent-command-excitation-v1"
...
def test_missing_history_and_partial_command_tail_preserve_slots():
    ...
    predictors.functions = {
        ...
        "original": lambda x, up, uf: np.ones((len(uf), 15)),
        "candidate": None,
    }
    arrays = dict(
...
def test_historical_interaction_globals_are_unchanged():
    ...
    assert private.ARMS == ("baseline", "original", "candidate")
    assert private.PREDICTORS == ("baseline", "original", "candidate", "hold")
...
def test_missing_original_rows_and_nonfinite_predictions_are_not_dropped(
    ...
    with pytest.raises(ValueError, match="complete planned roster"):
        experiment.validate_rows(
            "cascade", tmp_path, [r for r in rows if r["arm"] != "original"]
        )
...
@pytest.mark.parametrize("arm", ["baseline", "original"])
...
def test_both_research_fit_provenances_bind_baseline_and_anchor(tmp_path):
    ...
    experiment.freeze_files(base / "original", "seal.json", {})
    ...
    assert (
        provenance["original"]["baseline_seal"]
    ...
    assert (
        provenance["original"]["trusted_reference_bundle_sha256"]
        == experiment.protocol()["prior_quadratic_bundle"]["manifest_sha256"]
    )
    assert provenance["candidate"]["original_seal"] == experiment.digest(
        base / "original/seal.json"
    )
    assert provenance["candidate"]["excitation_seal"] == experiment.digest(
        base / "excitation/seal.json"
    )
    assert provenance["candidate"]["excitation_comparison_sha256"] == experiment.digest(
        base / "excitation.json"
    )
...
def test_prior_source_and_protocol_pins_reject_changes(monkeypatch):
    ...
    paths = [
        p["prior_interaction_protocol"]["path"],
        p["prior_quadratic_protocol"]["path"],
...
def test_quadratic_trust_anchor_rejects_coherent_local_reseal(monkeypatch, tmp_path):
    experiment.write_json(tmp_path / "run.json", {"files": {}})
    trusted = experiment.digest(tmp_path / "run.json")
    p = experiment.protocol()
    p["prior_quadratic_bundle"] = {"path": str(tmp_path), "manifest_sha256": trusted}
    monkeypatch.setattr(experiment, "protocol", lambda: p)
    assert experiment.trusted_quadratic() == tmp_path
    experiment.write_json(tmp_path / "run.json", {"files": {}, "changed": True})
    with pytest.raises(ValueError, match="trusted anchor"):
        experiment.trusted_quadratic()


def test_original_fit_cache_guard_covers_baseline_identity_and_arrays():
...
def test_both_excitation_datasets_are_checked_before_any_fit(monkeypatch, tmp_path):
...
def test_excitation_runtime_and_common_data_links_are_mandatory(monkeypatch, tmp_path):
    from glassbox.experimental import command_excitation_data as data

    runtime = {"source": "pinned"}
    base = tmp_path / "cascade"
    experiment.write_json(base / "data/seal.json", {"original": True})
    experiment.write_json(base / "excitation.json", {"matched": True})
    experiment.write_json(base / "excitation/records.json", [])
    experiment.freeze_files(
        base / "excitation",
        "seal.json",
        {
            "runtime": runtime,
            "common_data_seal": experiment.digest(base / "data/seal.json"),
        },
    )
    monkeypatch.setattr(data, "compare_excitation", lambda *args: {"matched": True})
    assert experiment.check_excitation("cascade", tmp_path, runtime) == {
        "matched": True
    }
    with pytest.raises(ValueError, match="runtime/source"):
        experiment.check_excitation("cascade", tmp_path, {"source": "changed"})
    experiment.write_json(base / "data/seal.json", {"original": False})
    with pytest.raises(ValueError, match="common data"):
        experiment.check_excitation("cascade", tmp_path, runtime)


def test_candidate_links_validate_excited_collection_not_baseline_cache(
    monkeypatch, tmp_path
):
    from types import SimpleNamespace

    from glassbox.experimental import command_excitation_fit as fitter

    private = experiment._implementation()
    monkeypatch.setattr(private, "_original_check_links", lambda *a, **k: None)
    monkeypatch.setattr(experiment, "check_sources", lambda: {})
    monkeypatch.setattr(experiment, "check_excitation", lambda *a: None)
    monkeypatch.setattr(experiment, "compare_baseline", lambda *a: {})
    monkeypatch.setattr(experiment, "compare_original", lambda *a: {})
    ...
    cache_checks = []
    monkeypatch.setattr(
        experiment, "_check_fit_cache", lambda m, b, arm: cache_checks.append(arm)
    )
    base = tmp_path / "cascade"
    for arm in experiment.ARMS:
        for filename, value in (
            ("outcome.json", {"model_available": True}),
            ("report.json", {}),
            ("contract.json", {}),
            ("matching.json", {}),
        ):
            experiment.write_json(base / arm / filename, value)
    experiment.write_json(base / "excitation/seal.json", {})
    collections = []

    def collect(directory):
        collections.append(directory)
        return "excited_collection", []

    monkeypatch.setattr(experiment.flight(), "collection_and_trajectories", collect)
    verified = []
    monkeypatch.setattr(
        fitter, "check_preparation", lambda *a, **k: verified.append((a, k))
    )
    experiment.check_links(tmp_path, "cascade")
    assert cache_checks == ["original"]
    assert collections == [base / "excitation"]
    assert verified[0][0] == (
        models["candidate"],
        "excited_collection",
        models["original"],
    )
    assert verified[0][1]["data_seal"] == experiment.digest(
        base / "excitation/seal.json"
    )
    assert (
        verified[0][1]["roles"]
        == experiment.protocol()["planned_automatic_roles"]["cascade"]
    )
