...
def test_parameter_prior_requires_fleet_coverage_for_target_controls(
    fixedwing_flight,
) -> None:
...
def test_configuration_specific_parameters_ignore_inapplicable_members(
    fixedwing_flight,
) -> None:
    trajectory = fixedwing_flight(4, 0.2)
    flap_spec = make_trajectory_spec(
        FIXED_WING_CONTROL_NAMES + ("flap",),
        family="fixedwing",
        observation_source="simulator_truth",
...
def test_initialize_belief_stales_inherited_error_evidence_unless_asserted(
    quadrotor_flight,
) -> None:
    from glassbox.belief.belief import (
