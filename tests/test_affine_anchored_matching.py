...
def prepared():
    ...
    sequence = anchored.initialize_candidate(
    ...
    report["recipe"] = copy.deepcopy(anchored.RECIPE)
    report["optimization"].update(
        mechanism=anchored.EXPERIMENT,
        minibatch_seed=10000,
        parameter_count=counts["candidate"],
    )
    report["matching"] = dict(
        baseline_fingerprint=public.fingerprint(),
        training_windows_fingerprint=anchored._window_fingerprint(train),
        development_windows_fingerprint=anchored._window_fingerprint(development),
...
def test_matching_replays_stably_without_fit_solve_rollout_or_mutation(
    ...
    for module, names in (
        ...
        (anchored, ("fit_candidate", "fit_candidate_sequence", "initialize_candidate")),
...
def test_every_changed_anchored_cached_array_is_rejected(trio, role, field):
    candidate = trio[1]
    ...
    with pytest.raises(ValueError, match=r"anchored .* cache"):
...
def test_cache_metadata_is_not_replaced_by_matching_values(trio, change):
    ...
    with pytest.raises(ValueError, match="anchored development cache"):
        matching.check_matched_models(*trio)


@pytest.mark.parametrize(
    "norm", [*shared.BASE_NORMS, "interaction_scale", "autonomous_scale"]
)
...
def test_anchored_semantic_mismatches_rejected(trio, change):
    candidate = trio[1]
    ...
    if change == "contract":
        ...
        candidate._model.params["foreign_parameter"] = np.zeros((3, 2))
    elif change == "update":
...
def test_public_or_quadratic_cannot_stand_in_for_anchored(trio, index):
...
def test_ablation_protocol_is_byte_pinned(monkeypatch):
    ...
    with pytest.raises(ValueError, match="frozen affine-anchored ablation protocol"):
        matching._protocol()
