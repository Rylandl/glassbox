...
dataset-specific motor-order override.
...
uses telemetry completeness to select the longest contiguous interval without
applying armed, landed, or local-height gates. Operational ULog ingestion keeps
the normal armed, airborne, and 0.2 m height defaults. The four recordings are
replicates from one vehicle, so they are useful real-ULog integration and
system-identification references, not evidence of cross-airframe transfer.
...
families, recording sessions, and now both a conventional tail configuration
and a three-control flying wing. The parameter artifacts remain airframe
specific; this evidence validates the shared interface and model family, not
zero-shot coefficient transfer. Multi-minute IDF sessions and complete X8
maneuvers still expose long-rollout instability, and flap-equipped or
configuration-changing aircraft remain unvalidated. More same-vehicle segments
are therefore lower value than another airframe/configuration. Use
`--include-ground` only with a model that includes ground-contact dynamics.
